...
local function ColorToString(Color)
	...
	if Color == nil then
		...
		String = "r"..Color["r"].." g"..Color["g"].." b"..Color["b"]
		if Color["a"] then
			String = String.." a"..Color["a"]
		end
		return String
	end
end
...
local function StringToColor(String)
	...
	if String == "nil" then
		...
		for _, k in pairs(Colors) do
			local Begin, _ = string.find(String, k)
			if Begin then
				local End, _ = string.find(String, " ", Begin)
				if not End then End = #String+1 end
...
local function Autobuy()
	if file.Exists("tttautobuyscriptdata/selection"..LocalPlayer():GetRole()..".txt","DATA") then
		local itemstobuy = LinesToTable(file.Read("tttautobuyscriptdata/selection"..LocalPlayer():GetRole()..".txt","DATA"))
...
local function Select(Equip, Selection) -- !rework!
	...
	elseif Selection == "Autobuy" or Selection == "Favorite" then
		NewEquip = {}
		local itemstobuy = LinesToTable(file.Read("ttt"..string.lower(Selection).."scriptdata/selection"..LocalPlayer():GetRole()..".txt","DATA"))
...
local function Search(Equip, SearchText) -- Search function new(test needed)
	if SearchText then
		LastSearched = SearchText
	else
		SearchText = LastSearched or ""
	end
	local ply = LocalPlayer()
	local items = {}
	
	for _, k in pairs(Equip) do
		if k and k["name"] and string.find(SafeTranslate(k["name"]):lower(), SearchText:lower()) then
...
local function Sortate(Equip, Sortation) -- Sortate new (test needed)
	...
	elseif Sortation == "Default" then
		table.sort(Equip, function(a, b) -- debug wrong order
			local a = a.id
			local b = b.id
...
local function IsItem(AutoOrFav, item) -- !rework!
	...
	local itemstobuy = LinesToTable(file.Read("ttt"..string.lower(AutoOrFav).."scriptdata/selection"..LocalPlayer():GetRole()..".txt","DATA"))
	if file.Exists("ttt"..string.lower(AutoOrFav).."scriptdata/selection"..LocalPlayer():GetRole()..".txt","DATA") then
		if itemstobuy[1] then 
			local i = 1
			while i <= #itemstobuy do
...
local function IsKindSelected(item) -- !rework!
...
local function EditList(AutoOrFav, item) -- !rework!
	...
	if IsItem(AutoOrFav, item) then
		local itemstobuy = LinesToTable(file.Read("ttt"..string.lower(AutoOrFav).."scriptdata/selection"..LocalPlayer():GetRole()..".txt","DATA"))
		file.Delete("ttt"..string.lower(AutoOrFav).."scriptdata/selection"..LocalPlayer():GetRole()..".txt")
		if itemstobuy[1] and item then 
			local itemstosave = {}
			local i = 1
			local i2 = 1
			while i <= #itemstobuy do
				if not (tostring(itemstobuy[i]) == tostring(item.id)) then
					file.Append("ttt"..string.lower(AutoOrFav).."scriptdata/selection"..LocalPlayer():GetRole()..".txt", itemstobuy[i].."\n")
				end
				i = i+1
			end
		end
	else
		file.Append("ttt"..string.lower(AutoOrFav).."scriptdata/selection"..LocalPlayer():GetRole()..".txt", item.id.."\n")
	end
end
...
local function DeleteMissingItems(AutoOrFav) -- Delete missing Items in saved lists !needs rework!
	local itemstobuy = LinesToTable(file.Read("ttt"..string.lower(AutoOrFav).."scriptdata/selection"..LocalPlayer():GetRole()..".txt","DATA"))
	local itemsloaded = GetEquipmentForRole(LocalPlayer():GetRole())
	if TTT2Active then
		local itemsloaded = TTT2GetEquipmentForRole(LocalPlayer():GetRole())
	end
	local i = 1
	local i2 = 1
	if file.Exists("ttt"..string.lower(AutoOrFav).."scriptdata/selection"..LocalPlayer():GetRole()..".txt","DATA") then
		...
		if 0 < #ItemsMissing then
			...
			while i3 <= #itemstobuy do
				if not (i3 == ItemsMissing[i4]) then 
					file.Append("ttt"..string.lower(AutoOrFav).."scriptdata/selection"..LocalPlayer():GetRole()..".txt", itemstobuy[i3].."\n")
	...
	tbl.owned.Check = function(s, sel)
		...
		for i in pairs(Weapons) do
			for i2 in pairs(EquipmentAll) do
				if SafeTranslate(EquipmentAll[i2]["name"]) == Weapons[i] then
					if EquipmentAll[i2]["kind"] == sel.kind then
...
local function TraitorMenuPopup()
	...
	if not ply:IsSpec() and not IsValid(ply) or not ((ply:GetRole() and TTTRoles[ply:GetRole()] and TTTRoles[ply:GetRole()].HasShop) or ply:IsActiveShopper()) or not ply:IsTerror() then -- if not IsValid(ply) or not TTTRoles[ply:GetRole()].HasShop or not ply:IsTerror() then
		return
	end
	...
	SearchText = false
	if TTT2Active then
		EquipmentAll = TTT2GetEquipmentForRole(LocalPlayer():GetRole())
	else
	...
	if TTT2Active and #EquipmentAll == 0 then
	...
	if StringToColor(TabColor:GetString()) then
		dsheet.Paint = function(self,w,h)
			draw.RoundedBox(5, 0, 21, w, h-21, StringToColor(TabColor:GetString()))
		end
	end
	...
	dlist:SetPadding(4)
	local function dlistwriting(Sortation, SearchText, Selected)
		local to_select = nil -- may cause bug
		if dlist then
			dlist:Clear()
		end
		
		local items = Select(Search(Sortate(EquipmentAll, Sortation), SearchText), Selected)
		for k, item in pairs(items) do -- do for every item
			...
			if item.material then
				...
				if ItemIsWeapon(item) and SlotOn:GetBool() then
					...
					local color = color_slot[ply:GetRole()]
					if ply.GetRoleData and ply:GetRoleData().color then
						color = ply:GetRoleData().color or Color(60,60,60)
					end
					...
					slot:SetIconProperties(StringToColor(SlotTextColor:GetString()), -- color slot text
										   "DefaultBold",
	...
	dSortByID.DoClick = function () dlistwriting("Default") end
	...
	dSortBySlot.DoClick = function () dlistwriting("Slot") end
...
hook.Add("TTTSettingsTabs", "TTTSettingsTab4TTTBetterTraitorMenu", function(dtabs)
	...
	function Settings_box:OnSelect(table_box, FirstSort, data_box)
		if FirstSort == "Default" then
			RunConsoleCommand( "ttt_bettermenu_defaultorder", "1" )
		elseif FirstSort == "Name" then
			RunConsoleCommand( "ttt_bettermenu_defaultorder", "2" )
		elseif FirstSort == "Slot" then
...
if TTT2Active then
	...
	local function WeaposhopPopup()
		...
		-- Close any existing traitor menu
		if weaponshopframe and IsValid(weaponshopframe) then weaponshopframe:Close() end
		...
		local function dlistwriting(Sortation, SearchText, Selected)
			...
			for k, item in pairs(items) do -- do for every item
				local ic = nil
				for _, k in pairs(EquipmentT) do
					if k.id == item.id then
						item.origin = "T"
					end
				end
				for _, k in pairs(EquipmentD) do
					if k.id == item.id then
				...
				timer.Simple( 0.1, function() dlist.OnActivePanelChanged(_ , _, ic ) end )
			end
		...
		dSortByOrigin.DoClick = function () dlistwriting("Origin") end
